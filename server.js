    ...
    function introduce(senderID, username) {
      sendTextMessage(
        ...
        "Hello, " +
          username +
          "!" +
          " I am your sympathizing, note-logging, media-suggesting AI.",
        function() {
          sendTextMessage(
            senderID,
            "Let's talk! Tap the menu below to uncover all of my abilities!",
            function() {}
          );
...
*/
    ...
    function sendNotes(recipientId, entries, callback) {
      ...
      if (elements.length > 0) {
        sendTextMessage(
          recipientId,
          "Gladly! Here are your recent entries...",
          function() {
            callSendAPI(messageData, callback);
          }
        );
        ...
        sendTextMessage(
          recipientId,
          "Apologies, but no notes were found!",
          function() {}
        );
      }
    }
    ...
    function sendGiphy(request, messageText, limit, senderID) {
      mgiphy.get(request, messageText, limit, function(url) {
        ...
        url
          ? sendMediaMessage(senderID, message1, function() {
              url
                ? sendTextMessage(senderID, "Powered by GIPHY", function() {})
                : console.log(" ============== ");
            })
          : sendTextMessage(
              senderID,
              "bummer... can't seem to find anything relevant! :(",
              function() {}
            );
      });
    }
    ...
    */
                    ...
                    if (response) {
                      ...
                      if (response.sentiment.document.label === "positive") {
                        ...
                        response.sentiment.document.label === "negative"
                      ) {
                        label = "-ve/" + emotion;
                      } else {
                        label = "meh/" + emotion;
                    ...
                    if (response.sentiment.document.label === "negative") {
                      sendTextMessage(
                        senderID,
                        "Oh, you seem upset... maybe this can cheer you up! ;)",
                        function() {
          ...
          if (response.result.action === "get-media") {
            ...
            var media_type =
              media_types[Math.floor(Math.random() * (media_types.length - 1))];
            ...
            var given_name =
              given_names[Math.floor(Math.random() * (given_names.length - 1))];
            ...
            var music_artist =
              music_artists[
                Math.floor(Math.random() * (music_artists.length - 1))
              ];
            ...
            var keywords = formulateKeywords(
              adjective,
              given_name,
              last_name,
              music_artist,
            ...
            switch (media_type) {
              case "#video":
                sendTextMessage(
                  senderID,
                  "OK! Now searching for something cool... 🤔",
                  function() {
                    // =========================================================
                    myoutube.getVideo(request, keywords, 30, function(data) {
                      ...
                      url
                        ...
                        : sendTextMessage(
                            senderID,
                            "bummer... nothing relevant was found! :(",
                            function() {}
                          );
          ...
          textObj
            ? sendTextMessage(senderID, textObj, function() {})
            : console.log("no response from API.AI");
          mediaObj
            ? sendMediaMessage(
                senderID,
                mediaObj.payload.facebook,
                function() {}
              )
    ...
    function receivedMessage(event) {
      ...
      if (postback === "getStarted") {
        ...
        resetContexts(app, senderID, "listening", function() {
          sendTextMessage(
            senderID,
            "Well, tell me what you're in the mood for! Any artist/genre/song/video/gif is possible :O !",
            function() {}
          );
        ...
        message !== undefined &&
        message.quick_reply !== undefined &&
        message.quick_reply.payload === "describeSelfie"
      ) {
        // ============================
        //     SELFIE GAME BUTTONS
        // ============================
        ...
        if (emotion[0] === "covered") {
          emoji = "🙈";
        }
        if (emotion[0] === "headwear") {
          emoji = "🙉";
        }
        ...
        sendTextMessage(
          senderID,
          "I think that " + color + " is a color that looks good on you! ;)",
          function() {
            if (glasses !== "" && glasses === "Eye") {
              sendTextMessage(
                senderID,
                "... also, I think these glasses look so cute!🤓 ",
                function() {}
              );
            } else if (glasses !== "" && glasses === "Sun") {
              sendTextMessage(
                senderID,
                "... also, wow! these are some cool shades! 😎",
                function() {}
              );
            }
        ...
        message !== undefined &&
        message.quick_reply !== undefined &&
        message.quick_reply.payload === "guessAge"
      ) {
        ...
        if (age === "none") {
          sendTextMessage(
            senderID,
            "It was not clear - face the camera and send another one! :P ",
            function() {}
          );
        } else
          sendTextMessage(
            senderID,
            "Well, I think you look " + age + " ! :P",
            function() {}
          );
      } else {
        console.log(
        ...
        if (messageText) {
          ...
          if (messageAttachments[0].type == "image") {
            ...
            if (action === "play-selfie") {
              ...
              mkairos.detect(
                ...
                function(values) {
                  if (values === "down") {
                    sendTextMessage(
                      senderID,
                      "Sorry, Can't check that now. Please try again later!",
                      function() {}
                    );
                  } else {
                    ...
                    if (faces.images) {
                      ...
                      var gender =
                        faces.images[0].faces[0].attributes.gender.type;
                      var glasses_type =
                        faces.images[0].faces[0].attributes.glasses; // "None", "eye", "sun"
              ...
              sendTextMessage(
                senderID,
                "Well, I can only check a photo if you wanna play the selfie game! If you do, Just ask me to! ;)",
                function() {}
              );
            }
...
*/
            ...
            sendTextMessage(
              senderID,
              "ok! now trying to understand your voice...",
              function() {}
            );
            ...
            maudio.transcribe(
              ...
              function(result) {
                ...
                var transcript = JSON.stringify(
                  mresult.results[0].alternatives[0].transcript
                );
                ...
                fs.unlink(
                  "./static/" + "" + senderID + "_" + timeOfMessage + ".mp4"
                );
                fs.unlink(
